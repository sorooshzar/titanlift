...
export default function SlideAccount({ answers, onFinish, saving }) {
  const handleOAuth = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col px-6 py-10">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-muted-foreground text-sm mb-8">Save your progress and access your data anywhere. Sign up with Google or Apple to get started instantly.</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="space-y-3">
        ...
        <div className="bg-secondary/50 rounded-2xl p-4 text-center mt-2">
          <p className="text-xs text-muted-foreground leading-relaxed">
            🔒 Your data is saved securely to your account. An account is required to use IronLog.
          </p>
        </div>
      </motion.div>
    </div>
