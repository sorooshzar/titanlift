...
function NutrientBar({ label, value, goal, color }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-semibold w-10 text-right">{Math.round(value)}{label.includes("kcal") ? "" : "g"}</span>
      {goal > 0 && <span className="text-[10px] text-muted-foreground w-8 text-right">{Math.round(pct)}%</span>}
    </div>
  );
}

export default function FoodDetailModal({ food, macroGoals, dailyTotals, mealType, date, onClose, onAdd }) {
  const [quantity, setQuantity] = useState(food.serving_size || 100);
        ...
        <div className="sticky top-0 bg-card z-10 px-5 pt-5 pb-3 border-b border-border/50">
          ...
          <div className="flex items-center gap-2 mt-3">
            <Input
              ...
              className="w-24 bg-secondary border-0 text-center font-bold"
            />
            <span className="text-sm text-muted-foreground">g</span>
            <div className="flex-1 grid grid-cols-4 gap-1 text-center">
              <div>
                <p className="text-[11px] font-bold" style={{ color: PROTEIN_COLOR }}>P</p>
                <p className="text-sm font-bold">{contributed.protein}g</p>
              </div>
              <div>
                <p className="text-[11px] font-bold" style={{ color: CARBS_COLOR }}>C</p>
                <p className="text-sm font-bold">{contributed.carbs}g</p>
              </div>
              <div>
                <p className="text-[11px] font-bold" style={{ color: FAT_COLOR }}>F</p>
                <p className="text-sm font-bold">{contributed.fat}g</p>
              </div>
              <div>
                <p className="text-[11px] font-bold" style={{ color: KCAL_COLOR }}>kcal</p>
                <p className="text-sm font-bold">{contributed.calories}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="px-5 pb-6 space-y-5 mt-4">
          ...
          <div>
            ...
            <div className="bg-secondary/40 rounded-xl p-3 space-y-2">
              {[
                { label: "Calories", value: food.calories_per_100g || 0, unit: "kcal", color: KCAL_COLOR },
          ...
          {/* Estimated vitamins & minerals (shown as placeholder bars based on food category) */}
          <div>
            <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wide mb-3">
            ...
            <p className="text-[9px] text-muted-foreground/50 mt-2">* Micronutrient estimates are approximate</p>
          </div>
        </div>
